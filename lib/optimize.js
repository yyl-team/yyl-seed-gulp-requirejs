    ...
    var
      rStream = stream
        ...
        .pipe(through.obj(function(file, enc, next) {
          ...
          if (iEnv.isCommit) {
            const fileMap = {};
            fileMap[file.relative] = iCnt;
            const mangled = UglifyJS.minify(fileMap, { fromString: true });
            if (!mangled || mangled.error) {
              iRes.trigger('msg', ['error', ['unable to minify JavaScript', mangled && mangled.error]]);
            }
            if (mangled.warnings) {
              mangled.warnings.forEach((warning) => {
                iRes.trigger('msg', ['warn', ['uglify warning', file.relative, warning]]);
              });
            }
            file.contents = Buffer.from(mangled.code, 'utf-8');
          } else {
            file.contents = Buffer.from(iCnt, 'utf-8');
          }
          this.push(file);
...
gulp.task('watch', gulp.parallel('all', (done) => {
  const watcher = gulp.watch(util.joinFormat(config.alias.srcRoot, '**/**.*'));
