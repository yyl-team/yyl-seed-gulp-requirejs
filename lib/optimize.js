...
const iRes = {
  on(eventName, fn) {
    if (!cache.resFn[eventName]) {
      cache.resFn[eventName] = [];
    }
    cache.resFn[eventName].push(fn);
  },
  trigger(eventName, argv) {
    const handleFns = cache.resFn[eventName];
    ...
    if (rPath.match(REG.HTML_IS_ABSLUTE) || rPath.match(REG.IS_HTTP)) {
      ...
      if (type === 'css-path' && !fs.existsSync(absPath)) {
        fn.log('msg', 'warn', [
          ...
          `  path not found: ${chalk.yellow(util.path.relative(PROJECT_PATH, absPath))}`
        ].join('\n'));
        return iPath;
      }
    }
        ...
        scss: function(iPath) {
          ...
          sourceFiles.forEach((iSource) => {
            ...
            iCnt.replace(/@import ["']([^'"]+)['"]/g, (str, $1) => {
              const myPath = util.joinFormat(path.dirname(iSource), $1 + (path.extname($1)?'': '.scss'));
          ...
          iCnt = frp.cssPathMatch(iCnt, (iPath) => {
            ...
            if (fs.existsSync(fn.hideUrlTail(util.path.join(dirname, rPath)))) { // 以当前文件所在目录为 根目录查找文件
              ...
              fn.log('msg', 'warn', [
                ...
                `  path not found: ${chalk.yellow(util.path.relative(PROJECT_PATH, fn.hideUrlTail(util.joinFormat(dirname, rPath))))}`
              ].join('\n'));
              return iPath;
            }
          });
  ...
  requirejs2dest: function(stream) {
    var
      rStream = stream
        ...
        .pipe(through.obj(function(file, enc, next) {
          let iCnt = file.contents.toString();
          iCnt = frp.jsPathMatch(iCnt, (iPath, type) => {
            const r = (rPath) => {
              switch (type) {
                case '__url':
                  return `'${rPath}'`;
...
// - html task
// + tpl task
gulp.task('tpl', ['pug-to-tpl-dest-task', 'tpl-to-dest-task'], (done) => {
  done();
});
...
// + js task
gulp.task('js', ['requirejs-task', 'jslib-task'], (done) => {
  done();
});
...
// + all
gulp.task('all', ['js', 'css', 'images', 'html', 'tpl'], (done) => {
  runSequence('inline-source', () => {
    if (!iEnv.silent) {
...
module.exports = function (iConfig, root) {
  config = iConfig;
  PROJECT_PATH = root;
